...
| **[Debug a remote target](remote-debugging.md)** | A live user-mode debugging session to connect to. | `open_windbg_remote`, `send_ctrl_break`, `run_windbg_cmd` |
| **[Debug a kernel target](kernel-debugging.md)** | A kernel debug connection (KDNET, pipe, serial). | `open_windbg_kernel`, `send_ctrl_break`, `run_windbg_cmd` |
