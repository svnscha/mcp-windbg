...
Tests that need a live `cdb.exe` (remote debugging) skip cleanly when it is absent, leaving
the unit coverage. Test dumps live in `src/mcp_windbg/tests/dumps/` via Git LFS (`git lfs pull`).
...
src/mcp_windbg/
  ...
  tests/             pytest suite; dumps/ holds test crash dumps (Git LFS)
